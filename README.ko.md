...
계정마다 한 번에 하나입니다. 이미 들어가 있거나 내 보고서 PR이 아직 열려 있으면 `submit`이 거절합니다.
...
거부된 PR의 댓글은 이유마다 무엇을 하면 되는지 한 줄씩 알려 줍니다. 대개는 최신 보고 도구로 보고서를 다시
쓰고 `submit --yes`를 다시 실행하는 것입니다. PR에 새 커밋을 올리면 다시 판정하며, 거부된 채 14일 동안
