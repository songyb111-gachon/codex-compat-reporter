...
| 2 | 거절: 이유와 할 일이 오류 출력에 있고, 아무것도 보내거나 쓰지 않았습니다. 예외는 둘입니다. `guide`는 보고서를 쓴 뒤에 거절될 수 있는데, 그때는 파일을 남기고 어디 있는지 알려 줍니다. 그리고 보내는 중에는(`submit --yes`, 또는 `send`를 입력한 뒤의 `guide`) 앞 단계가 GitHub에 이미 쓴 뒤(포크, 브랜치, 올린 파일) 다음 단계가 실패할 수 있습니다. 그때는 그 실행이 쓴 것을 메시지에 적으며, 다시 보내도 안전합니다. 포크는 그대로 두고, 브랜치를 프로젝트 main으로 되돌리고, 파일을 다시 더하기 때문입니다. 명령줄을 잘못 적었을 때도 2입니다. |
